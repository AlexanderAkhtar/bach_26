from argparse import ArgumentParser ##https://github.com/Daniil-Osokin/lightweight-human-pose-estimation-3d-demo.pytorch no change in draw needed
import json
import os
import csv
import time
            ...
            for pose_id in range(len(poses_2d)):
                ...
                for edge in body_edges:
                    for kpt_id in range(pose.shape[1]):
                        if pose[2, kpt_id] != -1:
                            d = time.time()
                            writer.writerow({"frame nummer": a, "x=": tuple(pose[0:2, kpt_id].astype(int))[0], "y=": tuple(pose[0:2, kpt_id].astype(int))[1], "time=": d-c})
            a = a + 1   
