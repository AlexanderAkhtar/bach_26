...
def draw_poses(img, poses_2d, g, c):
    for pose_id in range(len(poses_2d)):
        ...
        with open("result.csv", mode="a", newline='') as csvfile:
            ...
            for kpt_id in range(pose.shape[1]):
                if pose[2, kpt_id] != -1:
                    ...
                    d = time.time()
                    writer.writerow({"frame nummer": g, "x=": tuple(pose[0:2, kpt_id].astype(int))[0], "y=": tuple(pose[0:2, kpt_id].astype(int))[1], "time=": d-c})
